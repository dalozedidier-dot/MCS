...
def test_spectral_radius_known_2x2():
    ...
    assert spectral_radius([[2.0, 1.0], [1.0, 2.0]]) == pytest.approx(3.0, abs=1e-6)
...
def test_cascade_follows_spectral_threshold():
    ...
    recs = cascade_sweep(nodes, pattern, strengths=[0.0, 0.05, 1.2], n_steps=120)
...
def test_monte_carlo_reproducible_and_bounded():
    ...
    a = monte_carlo(cfg, n_steps=40, n_runs=50, sigma_L=0.1, sigma_R=0.05, sigma_B=0.05, seed=42)
    b = monte_carlo(cfg, n_steps=40, n_runs=50, sigma_L=0.1, sigma_R=0.05, sigma_B=0.05, seed=42)
...
def test_hysteresis_suppresses_false_alarms_monotonically():
    ...
    recs = false_alarm_study(cfg, ks=(1, 3, 6), n_steps=60, n_runs=40, sigma=0.12, seed=7)
...
def test_tornado_total_matches_margin_uncertainty():
    ...
    A = L + D
    C = theta * R * B
    ref = margin_uncertainty(
        t["M"], A, C, rel_err_A=0.1, rel_err_theta=0.1, rel_err_R=0.1, rel_err_B=0.1
    )
