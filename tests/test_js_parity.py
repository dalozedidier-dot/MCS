...
pytestmark = pytest.mark.skipif(node is None, reason="Node absent : parite JS non testable")
...
CASES = [
    # noyau pur
    (SimConfig(L=0.4, R=0.8, B=0.8, rho=0.7), {"L": 0.4, "R": 0.8, "B": 0.8, "rho": 0.7}),
    # substituabilite + charge en liste
    (
        SimConfig(L=[0.3, 0.5, 0.9, 0.4], R=0.75, B=0.6, rho=0.85, s=0.4),
        {"L_ramp": [0.3, 0.5, 0.9, 0.4], "R": 0.75, "B": 0.6, "rho": 0.85, "s": 0.4},
    ),
    ...
    (
        SimConfig(
            ...
            gamma=1.2,
            D_crit=0.6,
            theta_params=ThetaParams(theta0=1.0, theta_min=0.3, alpha=0.25, beta=0.15, tau=0.15),
        ),
        {
            "L": 0.45,
            "R": 0.7,
    ...
    (
        SimConfig(
            ...
            control=ControlParams(
                chi=0.15, kappa=0.4, eta=0.5, delta=0.05, u_max=1.5, gain=4.0, m_ref=0.2
            ),
            recovery=RecoveryParams(delta_D=0.3, delta_B=0.5, B_crit=0.4, R_min=0.2),
        ),
        {
            "L": 0.45,
            "R": 0.7,
            ...
            "recovery": {"delta_D": 0.3, "delta_B": 0.5, "B_crit": 0.4, "R_min": 0.2},
        },
    ),
    # incapacite critique C -> 0 via B = 0 (conventions de cas limites)
    (SimConfig(L=0.4, R=0.8, B=0.0, rho=0.7), {"L": 0.4, "R": 0.8, "B": 0.0, "rho": 0.7}),
]


@pytest.mark.parametrize(
    "py_cfg,js_cfg", CASES, ids=["noyau", "liste+s", "6.1+6.2", "6.3+6.5", "C=0"]
)
