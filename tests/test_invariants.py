...
@given(L=st.floats(0.01, 0.5), R=unit_open, B=unit_open, rho=st.floats(0.0, 0.95))
def test_debt_rest_level_is_fixed_point(L, R, B, rho):
    ...
    assert debt_update(D_star, L, R, B, C, rho) == pytest.approx(D_star, rel=1e-9)
    D = 0.0
    for _ in range(600):
...
_ORDER = ["coherence_viable", "tension_constructive", "saturation", "pre_rupture", "rupture"]
...
def test_classification_is_ordinal(M1, M2):
    ...
    assert _ORDER.index(classify(hi).value) <= _ORDER.index(classify(lo).value)


@given(ms=st.lists(st.floats(-1, 1, allow_nan=False), min_size=2, max_size=60), k=st.integers(1, 6))
...
@given(kappa=st.floats(0.05, 1.0), eta=st.floats(0.05, 1.0), B=unit_open, u=st.floats(0.0, 3.0))
...
cfg_strategy = st.builds(
    SimConfig,
    L=st.floats(0.05, 1.2),
    R=unit_open,
    B=unit_open,
    rho=st.floats(0.0, 0.98),
    s=unit,
    mu0=st.floats(0.0, 0.8),
    D_crit=st.floats(0.2, 1.5),
    theta_params=st.one_of(
        ...
        st.builds(
            ...
            alpha=st.floats(0.0, 0.6),
            beta=st.floats(0.0, 0.4),
