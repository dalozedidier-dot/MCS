...
def noisy(
    base, sigma: float, rng: random.Random, lo: float = 0.0, hi: float | None = None
) -> Callable[[int], float]:
...
class MonteCarloSummary:
    ...
    def as_dict(self) -> dict:
        return {
            ...
            "alert_rate": sum(a is not None for a in self.first_alert) / self.n_runs,
            "median_first_alert": statistics.median([a for a in self.first_alert if a is not None])
            if any(a is not None for a in self.first_alert)
            else None,
...
def monte_carlo(
    ...
    """
    order = [Zone.VIABLE, Zone.TENSION, Zone.SATURATION, Zone.PRE_RUPTURE, Zone.RUPTURE]
    ...
    for r in range(n_runs):
        ...
        if any(not math.isfinite(m) for m in res.M):
            out.diverged += 1
        alert = next((t for t, z in enumerate(res.zone) if rank[z] >= rank[alert_zone]), None)
...
def false_alarm_study(
    cfg: SimConfig,
    ks: Sequence[int] = (1, 2, 3, 5, 8),
    n_steps: int = 80,
    n_runs: int = 100,
    sigma: float = 0.10,
    seed: int = 0,
) -> list[FalseAlarmRecord]:
    ...
    for k in ks:
        ...
        for r in range(n_runs):
            ...
            raw_counts.append(sum(raw[i] != raw[i - 1] for i in range(1, len(raw))))
            conf_counts.append(sum(res.zone[i] != res.zone[i - 1] for i in range(1, len(res.zone))))
        tr, tc = statistics.mean(raw_counts), statistics.mean(conf_counts)
        records.append(FalseAlarmRecord(k, tr, tc, 0.0 if tr == 0 else 1.0 - tc / tr))
    return records
...
def oscillation_score(series: Sequence[float], burn_in: int = 10, tol: float = 1e-9) -> dict:
...
def spectral_radius(
    A: Sequence[Sequence[float]], iters: int = 500, tol: float = 1e-12, seed: int = 1
) -> float:
    ...
    for _ in range(iters):
        ...
        if norm == 0.0:
            return 0.0
        y = [v / norm for v in y]
        lam_new = sum(y[i] * sum(A[i][j] * y[j] for j in range(n)) for i in range(n))
...
def network_stability(rhos, coupling, R_list, B_list, D_crits) -> dict:
    ...
    rows = [
        rhos[i] + gains[i] * sum(coupling[i][j] / D_crits[j] for j in range(len(rhos)) if j != i)
        for i in range(len(rhos))
    ]
    return {"spectral_radius": sr, "stable": sr < 1.0, "small_gain_rows": rows}


def cascade_sweep(
    base_nodes: list[SimConfig],
    ...
    for s in strengths:
        ...
        results = simulate_network(NetworkConfig(nodes=base_nodes, coupling=coupling), n_steps)
        broken = sum(r.zone[-1] in (Zone.PRE_RUPTURE, Zone.RUPTURE) for r in results)
        records.append(
            {
                "strength": s,
                "spectral_radius": pred["spectral_radius"],
                "predicted_stable": pred["stable"],
                "nodes_broken": broken,
...
def sensitivity_tornado(
    L: float, D: float, theta: float, R: float, B: float, s: float = 0.0, rel_err: float = 0.10
) -> dict:
    ...
    return {"M": M, "contributions": contrib, "total": sum(contrib.values())}
