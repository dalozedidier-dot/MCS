...
def slow_degradation(n_steps: int = 60) -> SimResult:
    ...
    cfg = SimConfig(
        ...
        rho=0.85,
        D_crit=0.6,
        theta_params=ext.ThetaParams(theta0=1.0, theta_min=0.3, alpha=0.25, beta=0.15, tau=0.15),
    )
    return simulate(cfg, n_steps)
...
def control_overshoot(n_steps: int = 60, gain: float = 4.0) -> SimResult:
    ...
    cfg = SimConfig(
        ...
        mu0=0.3,
        D_crit=0.6,
        control=ext.ControlParams(
            chi=0.15, kappa=0.4, eta=0.5, delta=0.05, u_max=1.5, gain=gain, m_ref=0.2
        ),
    )
    return simulate(cfg, n_steps)
...
def chamber_recovery(n_steps: int = 60) -> SimResult:
    ...
    cfg = SimConfig(
        ...
        rho=0.8,
        D_crit=0.6,
        recovery=ext.RecoveryParams(delta_D=0.3, delta_B=0.5, B_crit=0.4, R_min=0.2),
    )
    return simulate(cfg, n_steps)
...
def coupled_cascade(n_steps: int = 60) -> list[SimResult]:
    ...
    return simulate_network(NetworkConfig(nodes=[a, b, c], coupling=coupling), n_steps)
