...
def load_ramp(L_min: float, L_max: float, n_up: int, plateau: int = 0) -> list[float]:
...
def hysteresis_loop(
    cfg: SimConfig, L_min: float = 0.15, L_max: float = 0.85, n_up: int = 40, plateau: int = 10
) -> HysteresisLoop:
    ...
    def run(d0: float) -> list[float]:
        cfg = SimConfig(
            ...
            theta_params=ThetaParams(
                theta0=theta0, theta_min=theta_min, alpha=alpha, beta=0.0, tau=1.0
            ),
        )
...
def regime_map(
    ...
    for i, rho in enumerate(rhos):
        grow_row, slope_row = [], []
        for a in alphas:
            sl = perturbation_slope(rho, a, R, B, theta0, theta_min, D_crit, L)
            slope_row.append(sl)
