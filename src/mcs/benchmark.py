...
1. GENERATEURS INDEPENDANTS DE LA LOI DE DETTE. Les trajectoires
   (L, R, B observes) sont produites par cinq familles dynamiques qui
   n'utilisent ni la dette ni la marge MCS. Certaines familles partagent
   toutefois l'hypothese generale qu'une erosion de R et B reduit une
   capacite latente : cette proximite structurelle est documentee, pas
   masquee. L'evenement est defini par un
...
la baseline la plus defavorable, avec une meme regle de calibration et
un taux de fausses alertes CIBLE commun. Les FPR observes en validation
restent publies tels quels et peuvent differer.
...
def _noisy(
    x: float, rng: random.Random, sigma: float, lo: float = 0.0, hi: float | None = None
) -> float:
...
def _hidden_event(load_true: list[float], K: list[float], w: int = EVENT_WINDOW) -> int | None:
    run = 0
...
def _false_shock(rng: random.Random, n: int) -> Trajectory:
    ...
    dur = min(rng.randint(2, 4), EVENT_WINDOW - 1)
...
def _recovery(rng: random.Random, n: int) -> Trajectory:
    ...
    load_true = [
        0.35 + 0.35 * min(t, 2 * peak - t) / peak if t < 2 * peak else 0.35 for t in range(n)
    ]
    K = [1.05] * n
    L = [_noisy(x, rng, 0.05) for x in load_true]
...
class DetectorResult:
    ...
    lead_quality: dict[str, float | int] = field(default_factory=dict)
...
def calibrate_threshold(scores_no_event: list[list[float]], target_fpr: float) -> float:
    ...
    idx = min(len(maxima) - 1, max(0, math.ceil((1.0 - target_fpr) * len(maxima)) - 1))
    return maxima[idx]
...
def evaluate_detector(name: str, thr: float, validation: list[Trajectory]) -> DetectorResult:
    ...
    for traj in validation:
        ...
        f = fam.setdefault(traj.family, {"tp": 0, "fp": 0, "fn": 0, "tn": 0, "leads": []})
    ...
    for k, f in fam.items():
        ...
        by_family[k] = {
            ...
            "median_lead": (float(statistics.median(f["leads"])) if f["leads"] else None),
            "n_event": np_,
            "n_no_event": nn_,
        }
    event_leads = list(lead_by_traj.values())
    n_events = len(event_leads)
    useful = sum(USEFUL_LEAD_MIN <= x <= USEFUL_LEAD_MAX for x in event_leads)
    too_early = sum(x > USEFUL_LEAD_MAX for x in event_leads)
    late_or_missed = sum(x < USEFUL_LEAD_MIN for x in event_leads)
    lead_quality: dict[str, float | int] = {
        "useful_min": USEFUL_LEAD_MIN,
        "useful_max": USEFUL_LEAD_MAX,
        "useful_count": useful,
        "too_early_count": too_early,
        "late_or_missed_count": late_or_missed,
        "useful_rate": useful / n_events if n_events else math.nan,
        "too_early_rate": too_early / n_events if n_events else math.nan,
        "late_or_missed_rate": (late_or_missed / n_events if n_events else math.nan),
    }
...
def paired_median_gain(a: DetectorResult, b: DetectorResult) -> float | None:
    ...
    if not seeds:
        return None
    diffs = [a.lead_by_traj.get(s, 0) - b.lead_by_traj.get(s, 0) for s in seeds]
    return float(statistics.median(diffs))


def paired_gain_distribution(a: DetectorResult, b: DetectorResult) -> list[int]:
    """Distribution appariee lead_a - lead_b, manques comptes a zero."""
    seeds = sorted(set(a.lead_by_traj) | set(b.lead_by_traj))
    return [a.lead_by_traj.get(s, 0) - b.lead_by_traj.get(s, 0) for s in seeds]


def _quantile(values: list[int], q: float) -> float:
    """Quantile lineaire deterministe, sans dependance externe."""
    if not values:
...
def bootstrap_median_ci(
    values: list[int], *, n_boot: int = 4000, alpha: float = 0.05, seed: int = 20260710
) -> tuple[float, float]:
    """IC percentile bootstrap de la mediane, reproductible."""
    if not values:
        raise ValueError("values ne peut pas etre vide")
    if n_boot < 1:
    ...
    n = len(values)
    medians = [
        float(statistics.median(values[rng.randrange(n)] for _ in range(n))) for _ in range(n_boot)
    ]
    return (
        _quantile([round(x * 1000) for x in medians], alpha / 2) / 1000,
        _quantile([round(x * 1000) for x in medians], 1 - alpha / 2) / 1000,
    )


def paired_gain_summary(a: DetectorResult, b: DetectorResult) -> dict:
    """Resume complet du gain apparie, y compris cas perdus et egalites."""
    diffs = paired_gain_distribution(a, b)
    if not diffs:
        return {"n": 0}
    ...
    return {
        ...
        "median": float(statistics.median(diffs)),
        "q1": _quantile(diffs, 0.25),
        "q3": _quantile(diffs, 0.75),
        "min": min(diffs),
        "max": max(diffs),
        "win_rate": sum(x > 0 for x in diffs) / len(diffs),
        "tie_rate": sum(x == 0 for x in diffs) / len(diffs),
        "loss_rate": sum(x < 0 for x in diffs) / len(diffs),
...
def run_benchmark(
    calibration_seeds: range = range(0, 150),
    ...
    if set(calibration_seeds) & set(validation_seeds):
        raise ValueError("graines de calibration et de validation doivent etre disjointes")
...
def benchmark_markdown(res: BenchmarkResult) -> str:
    lines = [
        ...
        "",
        f"Trajectoires generees par 5 familles n'utilisant ni la dette "
        f"ni la marge MCS (certaines partagent l'hypothese generale "
        f"qu'une erosion de R et B reduit une capacite latente). "
        f"Calibration : {res.n_calibration} trajectoires ; validation : "
        f"{res.n_validation} (graines disjointes). Seuils calibres par "
        f"la meme regle vers un FPR cible commun : {res.target_fpr:.0%}. "
        "Parametres des detecteurs pre-enregistres dans le code.",
        "",
        "| detecteur | sensibilite | FPR observe | precision | delai median (pas) |",
        "|---|---|---|---|---|",
    ]
    for d in sorted(res.detectors, key=lambda x: -(x.median_lead or -1)):
        lines.append(
    ...
    lines += [
        ...
        + ", ".join(
            f"`{k}` {v:+.1f}" for k, v in h["gains_apparies_par_baseline"].items() if v is not None
        )
        + ".",
        ...
        (
            f"Distribution appariee : victoire "
            f"{h['distribution_gain']['win_rate']:.1%}, egalite "
            f"{h['distribution_gain']['tie_rate']:.1%}, defaite "
            f"{h['distribution_gain']['loss_rate']:.1%}; IC95 bootstrap "
            f"de la mediane "
            f"[{h['distribution_gain']['median_ci95'][0]:+.1f}; "
            f"{h['distribution_gain']['median_ci95'][1]:+.1f}] pas."
        ),
        "",
    ...
    for fam, s in sorted(mcs.by_family.items()):
        if s["n_event"]:
            lines.append(
                f"| {fam} | {s['sensitivity']:.2f} | {s['median_lead']} | {s['n_event']} |"
            )
    lines += [
        "",
