...
mc = monte_carlo(cfg, n_steps=60, n_runs=300, sigma_L=0.10, sigma_R=0.05, sigma_B=0.05, seed=11)
fig, ax = plt.subplots(figsize=(7, 4))
for r in range(40):
    rng = random.Random(11 + r)
    c = replace(
        cfg,
        L=noisy(cfg.L, 0.10, rng),
...
for y, _lbl in ((0.30, "viable"), (0.10, "tension"), (0.05, "saturation"), (-0.05, "pre-rupture")):
...
recs = false_alarm_study(cfg_st, ks=(1, 2, 3, 5, 8), n_steps=80, n_runs=120, sigma=0.12, seed=7)
...
lines += [
    f"| {r.k} | {r.transitions_raw:.1f} | {r.transitions_confirmed:.1f} | {r.suppression:.0%} |"
    for r in recs
]
k3 = next(r for r in recs if r.k == 3)
lines += [
    "",
    f"k = 3 (defaut) supprime deja {k3.suppression:.0%} des "
...
nodes = [SimConfig(L=0.2, R=0.8, B=0.7, rho=0.9, D_crit=0.3) for _ in range(3)]
...
ax1.plot(
    [c["strength"] for c in casc],
    [c["spectral_radius"] for c in casc],
    "o-",
    color="#b8892b",
    label="rho(J)",
)
...
lines += [
    f"| {c['strength']} | {c['spectral_radius']:.3f} "
    f"| {c['predicted_stable']} | {c['nodes_broken']} |"
    for c in casc
]
lines += [
    "",
    "Le verdict spectral (exact) raffine la condition de somme "
...
(OUT / "falsification.md").write_text(falsification_report(recs), encoding="utf-8")
