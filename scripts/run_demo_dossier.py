...
cfg = SimConfig(
    ...
    D_crit=0.6,
    mu0=0.15,
    theta_params=ThetaParams(theta0=1.0, theta_min=0.4, alpha=0.3, beta=0.1, tau=0.2),
)
...
ax.plot(L_axis, loop.M_down, color="#b8892b", lw=2.2, label="descente (même charge)")
ax.fill_between(
    L_axis,
    loop.M_up,
    loop.M_down,
    color="#2f6b4f",
    alpha=0.12,
    label=f"mémoire du passage (aire {loop.loop_area:.3f})",
)
ax.plot(L_axis, temoin.M_up, color="grey", lw=1, ls=":")
ax.plot(
    L_axis, temoin.M_down, color="grey", lw=1, ls=":", label="témoin sans mémoire (ρ = 0, Θ figé)"
)
...
ax.set_title(
    f"Garde d'emballement : pente mesurée de la carte de dette (accord {rm.agreement:.1%})"
)
...
print(
    f"  boucle: aire={loop.loop_area:.3f} (témoin {temoin.loop_area:.4f}), "
