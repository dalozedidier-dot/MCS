...
(OUT / "benchmark.json").write_text(
    json.dumps(res.as_dict(), indent=2, ensure_ascii=False),
    encoding="utf-8")
(OUT / "benchmark.md").write_text(benchmark_markdown(res),
                                  encoding="utf-8")
...
colors = ["#2f6b4f" if n == "mcs_complet"
          else ("#b8892b" if n.startswith("mcs_") else "#9aa5a0")
          for n in names]
...
for b, s in zip(bars, sens, strict=True):
    ax.text(b.get_width() + 0.25, b.get_y() + b.get_height() / 2,
            f"sens. {s:.2f}", va="center", fontsize=8, color="#65726d")
ax.set_xlabel("delai median d'alerte (pas) - FPR cible identique 10 %")
ax.set_title("Benchmark aveugle : "
             f"{res.n_validation} trajectoires de validation, "
             "parametres pre-enregistres")
fig.tight_layout()
fig.savefig(OUT / "benchmark.png", dpi=150)
plt.close(fig)
shutil.copy(OUT / "benchmark.png", ROOT / "docs" / "assets" / "benchmark.png")
h = res.headline
print(f"gain median apparie (vs {h['baseline_la_plus_defavorable']}) : "
